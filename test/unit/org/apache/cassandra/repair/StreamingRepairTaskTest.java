...
public class StreamingRepairTaskTest extends AbstractRepairTest
{
    ...
    public void incrementalStreamPlan() throws NoSuchRepairSessionException
    {
        ...
        StreamingRepairTask task = new StreamingRepairTask(SharedContext.Global.instance, new SyncState(desc, desc.determanisticId()), desc, request.initiator, request.src, request.dst, request.ranges, desc.sessionId, PreviewKind.NONE, false);
    ...
    public void fullStreamPlan() throws Exception
    {
        ...
        RepairJobDesc desc = new RepairJobDesc(sessionID, nextTimeUUID(), ks, tbl, prs.getRanges());
        SyncRequest request = new SyncRequest(desc, PARTICIPANT1, PARTICIPANT2, PARTICIPANT3, prs.getRanges(), PreviewKind.NONE, false);
        StreamingRepairTask task = new StreamingRepairTask(SharedContext.Global.instance, new SyncState(desc, desc.determanisticId()), desc, request.initiator, request.src, request.dst, request.ranges, null, PreviewKind.NONE, false);
