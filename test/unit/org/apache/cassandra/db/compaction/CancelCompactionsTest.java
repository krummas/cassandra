...
public class CancelCompactionsTest extends CQLTester
{
    ...
    private void assertCompactionCount(int expected, TableMetadata metadata)
    {
        assertEquals(expected, getCompactions(metadata).size());
    }

    private List<CompactionInfo.Holder> getCompactions(TableMetadata metadata)
    {
        ...
        for (CompactionInfo.Holder h : CompactionManager.instance.active.getCompactions())
        {
            ...
            if (ci != null)
            {
                if (ci.getTableMetadata().equals(metadata))
                    compactions.add(h);
            }
        }
        return compactions;
