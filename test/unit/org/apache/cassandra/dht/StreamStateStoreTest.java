...
public class StreamStateStoreTest
{
    ...
    public void testUpdateAndQueryAvailableRanges()
    {
        ...
        session.addStreamRequest("keyspace1", ReplicaList.toDummyList(Collections.singleton(range)), ReplicaList.empty(), Collections.singleton("cf"));
        ...
        session.addStreamRequest("keyspace1", ReplicaList.toDummyList(Collections.singleton(range2)), ReplicaList.empty(), Collections.singleton("cf"));
        session.state(StreamSession.State.COMPLETE);
        store.handleStreamEvent(new StreamEvent.SessionCompleteEvent(session));
