...
public class StandaloneScrubber
{
    ...
    private static void checkManifest(CompactionStrategyManager manager, ColumnFamilyStore cfs, Collection<SSTableReader> sstables)
    {
        int maxSizeInMB = (int)((cfs.getCompactionStrategyManager().getMaxSSTableBytes()) / (1024L * 1024L));
        if (manager.getStrategies().size() == 2 && manager.getStrategies().iterator().next() instanceof LeveledCompactionStrategy)
        {
