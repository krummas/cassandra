...
public class DiagnosticSnapshotService
{
    ...
    private static class DiagnosticSnapshotTask implements Runnable
    {
        ...
        public void run()
        {
            try
            {
                ...
                {
                    assert isNormalized(ranges) : command + ", ranges = " + ranges;
                    cfs.snapshot(command.snapshot_name,
    ...
    @VisibleForTesting
    static boolean checkIntersection(List<Range<Token>> normalizedRanges, Token first, Token last)
    {
        int pos = Collections.binarySearch(normalizedRanges, new Bounds<>(first, last), Comparator.comparing(o -> o.left));

        if (pos < 0)
            pos = -pos - 1;

        Range<Token> prev = pos > 0 ? normalizedRanges.get(pos - 1) : null;
        Range<Token> next = pos < normalizedRanges.size() ? normalizedRanges.get(pos) : null;

        if (prev != null)
        {
            if (prev.right.compareTo(first) >= 0 || prev.right.isMinimum())
                return true;
            if (prev.left.equals(prev.right))
                return true;
        }
        if (next != null)
        {
            if (next.left.compareTo(last) < 0)
                return true;
            if (next.left.equals(next.right))
                return true;
        }
        return false;
    }
    ...
    @VisibleForTesting
    static boolean isNormalized(List<Range<Token>> ranges)
    {
        Token lastRight = null;
        for (int i = 0, isize=ranges.size(); i < isize; i++)
        {
            Range<Token> r = ranges.get(i);
            if (r.isWrapAround() && !r.right.isMinimum())
                return false;

            if (lastRight != null && r.left.compareTo(lastRight) < 0)
                return false;
            lastRight = r.right;
        }
        return true;
