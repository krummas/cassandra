...
/**
 * See {@link NonBlockingQueue}
 *
 * A mostly-readonly view on a {@link NonBlockingQueue}. Advancing this view will not affect the position of the queue
 * it is a view of, or other views on the queue, however successful remove operations will *generally* (though regular
 * Iterator removals are not guaranteed to) be reflected in the original queue, and all other views. safeRemove() and
 * NBQV.remove() are guaranteed to be reflected in all views, and is an atomic operation across all views,
 * i.e. only one such operation may return true out of identical operations on any views on the queue (including the original).
 *
 * This is kept as a distinct class from NBQ to ensure that only one appendable copy of any queue exists. Whilst
 * this isn't strictly necessary for function, it makes reasoning about behaviour easier and prevents accidentally
 * suboptimal behaviour (e.g. with tail pointers)
 */
public class NonBlockingQueueView<V> implements Iterable<V>
{
    ...
    static final class Node<V>
    {
         ...
         */
        // TODO: this does not need to be volatile for isBefore(), could use unsafe to only perform volatile accesses when necessary
    ...
    protected static final AtomicReferenceFieldUpdater<NonBlockingQueueView, Node> headUpdater = AtomicReferenceFieldUpdater.newUpdater(NonBlockingQueueView.class, Node.class, "head");
    ...
    protected final boolean advanceIfHead(Node expectedHead)
    {
        ...
        Node head = this.head;
        Node next = head.next;
        if (next != expectedHead)
            return false;
        return headUpdater.compareAndSet(this, head, next);
    }
     ...
     *
     * this method is ordinarily more efficient that simply successively calling advance() and add(),
    ...
    public int drainTo(List<? super V> list, int maxItems)
    {
        ...
        fast: while (maxItems > 0)
        {
            ...
            int skipped = 0;
            while (this.head != head || !headUpdater.compareAndSet(this, head, tail))
            {
                head = head.next;
    ...
    {
        SafeRemoveIterator<V> iter = iterator();
        while (iter.hasNext())
        {
            if (item.equals(iter.next()) && iter.safeRemove())
                return true;
        }
        return false;
    }
     ...
     */
    class SliceIterator implements SafeRemoveIterator<V>
    {
        ...
        public boolean hasNext()
        {
            while (true)
            {
                ...
                if (end != null && next.state - end.state > 0)
                    return false;
                if (!next.isDeleted())
                    return true;
                ...
                NonBlockingQueueView.this.unsafeRemove(cur, next, next2);
                next = next2;
            }
        }
        ...
        public void remove()
        {
            ...
            NonBlockingQueueView.this.unsafeRemove(pred, cur, next);
            cur = pred;
            pred = null;
        }
