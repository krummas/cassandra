...
public class NanoTimeToCurrentTimeMillis
{
    ...
    private static volatile long TIMESTAMP_BASE[] = new long[] { Clock.instance.currentTimeMillis(), System.nanoTime() };

    /*
     * Clock.instance.currentTimeMillis() is 25 nanoseconds. This is 2 nanoseconds (maybe) according to JMH.
     ...
     *
     * These timestamps don't order with Clock.instance.currentTimeMillis() because currentTimeMillis() can tick over
    ...
    private static void updateTimestampBase()
    {
        TIMESTAMP_BASE = new long[] {
                                    Math.max(TIMESTAMP_BASE[0], Clock.instance.currentTimeMillis()),
