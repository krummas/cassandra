 ...
 *
 */
public class RangeAwareCompactionStrategy extends AbstractCompactionStrategy
{
    ...
    private volatile DiskBoundaries diskBoundaries = null;
    private boolean shouldDefragment = false;
    ...
    {
        // todo: investigate if we still need all this now that we have DiskBoundaries
        if (diskBoundaries == cfs.getDiskBoundaries())
            return;
        diskBoundaries = cfs.getDiskBoundaries();

        Collection<Range<Token>> lr = diskBoundaries.localRanges;
    ...
    private AbstractCompactionStrategy getStrategyFor(int index)
    {
        ...
        {
            AbstractCompactionStrategy strat = cfs.createCompactionStrategyInstance(compactionParams);
            strategies.set(index, strat);
            shouldDefragment = strat.shouldDefragment(); //todo: all instances are of the same class currently, that could change though
        }
