...
public class Scrubber implements Closeable
{
    ...
    public Scrubber(ColumnFamilyStore cfs, SSTableReader sstable, boolean skipCorrupted, OutputHandler outputHandler, boolean isOffline) throws IOException
    {
        ...
        this.destinations = cfs.directories.getDirectoryForCompactedSSTables();
    ...
    public void scrub()
    {
        ...
        try
        {
            ...
            writer = CompactionManager.createWriter(cfs, destinations, expectedBloomFilterSize, sstable.getSSTableMetadata().repairedAt, sstable);
            ...
            if (writer.hasDataWritten())
            {
                ...
                newSstables = writer.closeAndOpenReader(sstable.maxDataAge, repairedAt);
            }
        }
        catch (Throwable t)
        {
        ...
        if (!outOfOrderRows.isEmpty())
        {
            ...
            long repairedAt = badRows > 0 ? ActiveRepairService.UNREPAIRED_SSTABLE : sstable.getSSTableMetadata().repairedAt;
            SSTableWriterInterface inOrderWriter = CompactionManager.createWriter(cfs, destinations, expectedBloomFilterSize, repairedAt, sstable);
            for (Row row : outOfOrderRows)
                inOrderWriter.append(row.key, row.cf);
            newInOrderSstables = inOrderWriter.closeAndOpenReader(sstable.maxDataAge);
            outputHandler.warn(String.format("%d out of order rows found while scrubbing %s; Those have been written (in order) to a new sstable (%s)", outOfOrderRows.size(), sstable, newInOrderSstables));
        }
