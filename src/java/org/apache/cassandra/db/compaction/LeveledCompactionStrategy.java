...
public class LeveledCompactionStrategy extends AbstractCompactionStrategy implements INotificationConsumer
{
    ...
    public AbstractCompactionTask getMaximalTask(int gcBefore)
    {
        while (true)
        {
            Pair<Boolean, AbstractCompactionTask> nextTask = getMaximalTaskInternal(gcBefore);
            if (nextTask.left)
                return nextTask.right;
        }
    }

    private synchronized Pair<Boolean, AbstractCompactionTask> getMaximalTaskInternal(int gcBefore)
    {
        OperationType op;
        ...
        if (cfs.getDataTracker().markCompacting(candidate.sstables))
        {
            ...
            return Pair.<Boolean, AbstractCompactionTask>create(true, newTask);
        }
