...
public class CompactionTask extends AbstractCompactionTask
{
    ...
    protected void runWith(File sstableDirectory) throws Exception
    {
        ...
        List<SSTableReader> sstables = writer.finished();
        ...
        logger.info(String.format("Compacted %d sstables to [%s].  %,d bytes to %,d (~%d%% of original) in %,dms = %fMB/s.  %,d total partitions merged to %,d.  Partition merge counts were {%s}",
                                  sstables.size(), builder.toString(), startsize, endsize, (int) (ratio * 100), dTime, mbps, totalSourceRows, totalKeysWritten, mergeSummary.toString()));
