...
public class SizeTieredCompactionStrategy extends AbstractCompactionStrategy
{
    ...
    private List<SSTableReader> getNextBackgroundSSTables(final int gcBefore)
    {
        ...
        for (List<SSTableReader> bucket : getBuckets(createSSTableAndLengthPairs(candidates), options.bucketHigh, options.bucketLow, options.minSSTableSize))
        {
     ...
     * @return
     */
    private List<List<SSTableReader>> splitBucket(List<SSTableReader> bucket)
    {
        List<List<SSTableReader>> splitBuckets = new ArrayList<>();
        Set<SSTableReader> curBucket = new HashSet<>();
        Set<SSTableReader> bucketSet = new HashSet<>(bucket);
     ...
     */
    private Set<SSTableReader> overlapping(SSTableReader r, Set<SSTableReader> rs)
    {
        ...
        while (candidates.peek() != null)
        {
            ...
            {
                if (intersects(candidate, sstable))
                {
    ...
    private boolean intersects(SSTableReader a, SSTableReader b)
    {
        Range<RowPosition> aRange = new Range<RowPosition>(a.first.token.minKeyBound(), a.last.token.maxKeyBound());
        Range<RowPosition> bRange = new Range<RowPosition>(b.first.token.minKeyBound(), b.last.token.maxKeyBound());
    ...
    public Collection<AbstractCompactionTask> getMaximalTask(final int gcBefore)
    {
        ...
        final List<SSTableReader> repaired = new ArrayList<>();
        ...
        List<List<SSTableReader>> splitRepaired = splitBucket(repaired);
        splitRepaired.addAll(splitBucket(unrepaired));
        //List<List<SSTableReader>> splitUnrepaired = splitBucket(unrepaired);
        //Iterable<List<SSTableReader>> all = Iterables.concat(splitRepaired, splitUnrepaired);
        logger.info("Created {} separate buckets for the compaction.", Iterables.size(splitRepaired));
        return Lists.newArrayList(Iterables.transform(splitRepaired, new Function<List<SSTableReader>, AbstractCompactionTask>()
        {
            @Override
            public AbstractCompactionTask apply(List<SSTableReader> sstables)
            {
