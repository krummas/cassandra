...
public class SizeTieredCompactionStrategy extends AbstractCompactionStrategy
{
    ...
    private List<List<SSTableReader>> splitBucket(List<SSTableReader> bucket)
    {
        ...
        for (SSTableReader r : bucket)
        {
            if (curBucket.contains(r))
                continue;
            Set<SSTableReader> overlap = overlapping(r, new HashSet<>(bucket));
    ...
    private Set<SSTableReader> overlapping(SSTableReader r, Set<SSTableReader> rs)
    {
        ...
        for (SSTableReader candidate : rs)
        {
            if (intersects(r, candidate))
            {
                retSet.add(candidate);
                retSet.addAll(overlapping(candidate, Sets.difference(rs, retSet)));
            }
        }
        return retSet;
    }
    ...
    {
        Range<RowPosition> aRange = new Range<RowPosition>(a.first, a.last);
        Range<RowPosition> bRange = new Range<RowPosition>(b.first, b.last);
