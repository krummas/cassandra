...
public class SizeTieredCompactionStrategy extends AbstractCompactionStrategy
{
    ...
    private List<SSTableReader> getNextBackgroundSSTables(final int gcBefore)
    {
        ...
        Set<SSTableReader> l1 = new HashSet<>();
        ...
        List<List<SSTableReader>> l0buckets = new ArrayList<>();
        List<List<SSTableReader>> l1buckets = new ArrayList<>();
        for (List<SSTableReader> bucket : getBuckets(createSSTableAndLengthPairs(l0), options.bucketHigh, options.bucketLow, options.minSSTableSize))
        {
            List<List<SSTableReader>> splitBuckets = splitBucket(bucket);
            logger.debug("Compaction bucket {} got split into {} buckets ({}) due to non-overlapping sstables", bucket, splitBuckets.size(), splitBuckets);
            l0buckets.addAll(splitBuckets);
        }

        for (List<SSTableReader> bucket : getBuckets(createSSTableAndLengthPairs(l1), options.bucketHigh, options.bucketLow, options.minSSTableSize))
        {
            List<List<SSTableReader>> splitBuckets = splitBucket(bucket);
            logger.debug("Compaction bucket {} got split into {} buckets ({}) due to non-overlapping sstables", bucket, splitBuckets.size(), splitBuckets);
            l1buckets.addAll(splitBuckets);
        }

        updateEstimatedCompactionsByTasks(l0buckets, l1buckets);

        List<SSTableReader> mostInteresting = mostInterestingBucket(l0buckets, minThreshold, maxThreshold);
        if (!mostInteresting.isEmpty())
            return mostInteresting;

        mostInteresting = mostInterestingBucket(l1buckets, minThreshold, maxThreshold);
        if (!mostInteresting.isEmpty())
            return mostInteresting;
    ...
    /**
     * Each in-bucket contains files of similar size, the returned buckets might contain several different buckets
     * with similar size, but it is guaranteed that the files in those buckets all intersect.
     *
     * Main use is when splitting ranges out on different disks, to make sure we don't end up in
     * compaction loops (say we had 4 disks, we would always be recompacting the files generated by
     ...
     * @return
     */
    public static List<List<SSTableReader>> splitBucket(Iterable<SSTableReader> bucket)
    {
        List<List<SSTableReader>> splitBuckets = new ArrayList<>();
        Set<SSTableReader> curBucket = Sets.newHashSet();
        Set<SSTableReader> bucketSet = Sets.newHashSet(bucket);
        for (SSTableReader r : bucket)
        {
            if (curBucket.contains(r))
                continue;
            Set<SSTableReader> overlap = overlapping(r, Sets.difference(bucketSet, curBucket));
            curBucket.addAll(overlap);
            splitBuckets.add(new ArrayList<>(overlap));
        }
        return splitBuckets;
    }

    /**
     * all overlapping sstables in rs, starting with r
     *
     * TODO: optimize, we can sort and break the loops early etc
     */
    private static Set<SSTableReader> overlapping(SSTableReader r, Set<SSTableReader> rs)
    {
        Set<SSTableReader> retSet = new HashSet<>();
        Queue<SSTableReader> candidates = new ArrayDeque<>();
        candidates.add(r);
        retSet.add(r);
        while (candidates.peek() != null)
        {
            SSTableReader candidate = candidates.poll();
            for (SSTableReader sstable : Sets.difference(rs, retSet))
            {
                if (candidate.intersects(sstable))
                {
                    if(retSet.add(sstable))
                        candidates.offer(sstable);
                }
            }
        }
        return retSet;
    ...
    static Pair<List<SSTableReader>, Double> trimToThresholdWithHotness(List<SSTableReader> bucket, int maxThreshold)
    {
        ...
        // we know the sstables in bucket are intersecting with each other
        // but we could have a situation with 1000 sstables where 999 are
        // not overlapping, but the final sstable overlaps them all.
        // Then we do not want to create a compaction that takes the first 32 non-intersecting
        // sstables and compacts into 32 new ones (we would compact "forever")
        ...
        do
        {
            int pruneLimit = maxThreshold;
            if (i + pruneLimit > bucket.size())
                pruneLimit = bucket.size() - i;

            prunedBucket = bucket.subList(i, i + pruneLimit);
            i++;

        } // if the bucket splits in as many parts as there are input sstables, we have no overlap and a compaction would be pointless:
        while (i < bucket.size() && splitBucket(prunedBucket).size() == prunedBucket.size());
    ...
    public Collection<AbstractCompactionTask> getMaximalTask(final int gcBefore)
    {
        ...
        Pair<Set<SSTableReader>, Set<SSTableReader>> repairedUnrepaired = splitInRepairedAndUnrepaired(sstables);
        List<Set<SSTableReader>> splitRepaired = new ArrayList<>();
        if (repairedUnrepaired.left.size() > 0)
            splitRepaired.add(repairedUnrepaired.left);
        if (repairedUnrepaired.right.size() > 0)
            splitRepaired.add(repairedUnrepaired.right);

        return Lists.newArrayList(Iterables.transform(splitRepaired, new Function<Set<SSTableReader>, AbstractCompactionTask>()
        {
            @Override
            public AbstractCompactionTask apply(Set<SSTableReader> sstables)
            {
    ...
    private void updateEstimatedCompactionsByTasks(List<List<SSTableReader>> l0tasks, List<List<SSTableReader>> l1tasks)
    {
        int n = 0;
        for (List<SSTableReader> bucket: l0tasks)
        {
            if (bucket.size() >= cfs.getMinimumCompactionThreshold())
                n += Math.ceil((double)bucket.size() / cfs.getMaximumCompactionThreshold());
        }
        for (List<SSTableReader> bucket: l1tasks)
        {
            if (bucket.size() >= cfs.getMinimumCompactionThreshold())
                n += Math.ceil((double)bucket.size() / cfs.getMaximumCompactionThreshold());
        }
