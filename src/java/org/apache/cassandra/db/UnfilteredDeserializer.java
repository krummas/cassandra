    ...
    public static class OldFormatDeserializer extends UnfilteredDeserializer
    {
        ...
        static class UnfilteredIterator implements PeekingIterator<Unfiltered>
        {
            ...
            private Unfiltered readRow(LegacyLayout.LegacyAtom first)
            {
                ...
                while (atoms.hasNext())
                {
                    ...
                    else
                    {
                        ...
                        atoms.next(); // consume since we had only just peeked it so far
                    }
                }
            ...
            private static class AtomIterator implements PeekingIterator<LegacyLayout.LegacyAtom>
            {
                ...
                private final Queue<LegacyLayout.LegacyAtom> outOfOrderAtom;
                ...
                private AtomIterator(Supplier<LegacyLayout.LegacyAtom> atomReader, CFMetaData metadata)
                {
                    ...
                    this.outOfOrderAtom = new PriorityQueue<>(atomComparator);
                }

                public boolean hasNext()
                {
                    if (readerExhausted)
                        return !outOfOrderAtom.isEmpty(); // We have to return out of order atoms when reader exhausts

                    // Note that next() and peek() assumes that next has been set by this method, so we do it even if
                    // we have some outOfOrderAtom stacked up.
                    if (next == null)
                        next = atomReader.get();

                    readerExhausted = next == null;
                    return !readerExhausted || !outOfOrderAtom.isEmpty();
                }
                ...
                {
                    return !outOfOrderAtom.isEmpty()
                           && (next == null || atomComparator.compare(outOfOrderAtom.peek(), next) <= 0);
                }
