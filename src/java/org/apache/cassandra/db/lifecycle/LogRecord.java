...
final class LogRecord
{
    ...
    public static Map<String, List<File>> getExistingFiles(Set<String> absoluteFilePaths)
    {
        ...
        Map<File, TreeSet<String>> pathToFileNamePrefix = new HashMap<>();
        for (String absolutePath : absoluteFilePaths)
        {
            Path fullPath = Paths.get(absolutePath);
            Path path = fullPath.getParent();
            if (path != null)
                pathToFileNamePrefix.computeIfAbsent(path.toFile(), (k) -> new TreeSet<>()).add(fullPath.getFileName().toString());
        }

        FilenameFilter ff = (dir, name) -> {
            TreeSet<String> dirSet = pathToFileNamePrefix.get(dir);
        ...
        for (File f : pathToFileNamePrefix.keySet())
            f.listFiles(ff);
