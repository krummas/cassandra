...
public class Memtable
{
    ...
    public List<FlushRunnable> flushRunnables()
    {
        ...
        Directories.DataDirectory[] locations = cfs.directories.getFlushLocations();
        List<Token> boundaries = cfs.partitioner.splitRanges(localRanges, locations.length);
        List<FlushRunnable> runnables = new ArrayList<>();
        ReplayPosition pos = lastReplayPosition.get();
        if (boundaries == null)
        {
            runnables.add(new FlushRunnable(pos, locations[0]));
            return runnables;
        }
